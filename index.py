...
UTC8 = timezone(timedelta(hours=8)) # UTC+8 timezone
...
def log_event(event_type, message):
    ...
    log_file = log_dir / LOG_FILENAME  # <-- Uses the configured filename

    timestamp = datetime.now(UTC8).strftime("%Y-%m-%d %H:%M:%S")
    log_line = f"[{timestamp} UTC+8] [{event_type.upper()}] {message}\n"

    with open(log_file, "a") as f:
        f.write(log_line)

    print(f"📝 Logged: {event_type} → {message} ({timestamp} UTC+8)")
#======================================#
    ...
    while True:
        try:
            ...
            if has_open_pos:
                ...
                if current_position:
                    print(f"⏳ Still in position {current_position['trade_direction'].upper()} @ Entry: {current_position['entry_price']}")

            else:       
                if current_position:
                    print(f"❌ Stop loss hit for {current_position['trade_direction'].upper()}")
                    log_event("TPSL", f"Position TPSL triggered for {current_position['trade_direction'].upper()} @ Entry: {current_position['entry_price']})")
                    open_position = False
                    current_position = None
                    continue
...
def configure_settings():
    ...
    """Interactive configuration setup before running the trading monitor."""
    global LIVE_MODE, USE_TOR, proxies, USD_AMOUNT, LEVERAGE, TPSL, TP, SL, SMALL_INTERVAL, LARGE_INTERVAL, REFRESH_SEC, LIMIT, LOG_FILENAME 

    print("\n=== ⚙️ Trading Configuration Setup ===")

    # === Mode Selection ===
    LIVE_MODE = input("Run in LIVE mode? (y/n) [default=n]: ").strip().lower() == 'y'

    # === Tor Proxy ===
    USE_TOR = input("Use Tor Proxy? (y/n) [default=n]: ").strip().lower() == 'y'
    proxies = TOR_PROXIES if USE_TOR else None

    # === Trading Configurations ===
    try:
        USD_AMOUNT = float(input(f"USD amount per trade [default={USD_AMOUNT}]: ") or USD_AMOUNT)
        LEVERAGE = int(input(f"Leverage [default={LEVERAGE}]: ") or LEVERAGE)
    except ValueError:
        print("⚠️ Invalid input, using default values.")

    TPSL = input("Enable Take Profit / Stop Loss? (y/n) [default=y]: ").strip().lower() != 'n'

    if TPSL:
        try:
            TP = float(input(f"Take Profit % (e.g., 0.04 = 4%) [default={TP}]: ") or TP)
            SL = float(input(f"Stop Loss % (e.g., 0.02 = 2%) [default={SL}]: ") or SL)
        except ValueError:
            print("⚠️ Invalid input, using default TP/SL values.")

    # === Candle Settings ===
    SMALL_INTERVAL = input(f"Small timeframe (e.g., 15m, 1H) [default={SMALL_INTERVAL}]: ") or SMALL_INTERVAL
    LARGE_INTERVAL = input(f"Large timeframe (e.g., 4H, 1D) [default={LARGE_INTERVAL}]: ") or LARGE_INTERVAL

    try:
        REFRESH_SEC = int(input(f"Refresh seconds [default={REFRESH_SEC}]: ") or REFRESH_SEC)
        LIMIT = int(input(f"Number of candles to fetch [default={LIMIT}]: ") or LIMIT)
    except ValueError:
        print("⚠️ Invalid number input, using default refresh/candle settings.")

    # === Log Filename ===
    log_name = input(f"Log filename [default={LOG_FILENAME}]: ").strip()
    if log_name:
        # Automatically add .log if user omits it
    ...
    print(f"  • USD per trade: {USD_AMOUNT}")
    print(f"  • Leverage: {LEVERAGE}x")
    print(f"  • TP/SL: {'Enabled' if TPSL else 'Disabled'}")
    if TPSL:
        print(f"    - Take Profit: {TP*100:.1f}% | Stop Loss: {SL*100:.1f}%")
    print(f"  • Small TF: {SMALL_INTERVAL} | Large TF: {LARGE_INTERVAL}")
    print(f"  • Refresh: {REFRESH_SEC}s | Candles: {LIMIT}")
...
#======================================#
def choose_monitoring_method():
    """Ask the user which monitoring method to run (simulation or live)."""
    print("\n=== 📊 Choose Monitoring Method ===")
    print("1️⃣  Simulate Trading (no real orders)")
    print("2️⃣  Live/Demo Trading (real API orders)")
